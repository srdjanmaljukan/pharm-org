...
export default function TargetsClient({ items: initialItems, currentMonth, currentYear }: Props) {
  ...
  const handleDeleteItem = async (id: string) => {
    if (!confirm("Ukloniti ovaj artikal?")) return;
    const result = await deleteTargetItem(id);
    if (result.success) {
      setItems((prev) => prev.filter((i) => i.id !== id));
    }
  };

  // ── Unos prodaje (bez PIN-a) ───────────────────────────────────────────────

  const handleAddSale = async (targetItemId: string) => {
    const sf  = saleForm[targetItemId];
    const qty = Number(sf?.qty || 0);
    if (!qty || qty < 1) return;

    const result = await addSaleEntry(targetItemId, qty, "", sf?.note || "");
    if (result.success && result.entry) {
      ...
      setSaleForm((prev) => ({ ...prev, [targetItemId]: { qty: "", note: "" } }));
    }
  };

  const handleDeleteSale = async (entryId: string, targetItemId: string) => {
    if (!confirm("Obrisati ovaj unos?")) return;
    const result = await deleteSaleEntry(entryId);
    if (result.success) {
      setItems((prev) =>
        prev.map((item) =>
          item.id === targetItemId
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <div className="space-y-3">
        ...
        {/* Navigacija po mjesecima — u posebnom redu */}
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={() => navigateMonth(-1)} disabled={loading}>
            <ChevronDown className="w-4 h-4 rotate-90" />
          </Button>
          <span className="text-sm font-medium flex-1 text-center sm:flex-none sm:min-w-[140px]">
            {MONTHS[month - 1]} {year}
          </span>
          <Button variant="outline" size="icon" onClick={() => navigateMonth(1)} disabled={loading}>
        ...
        <div className="rounded-xl border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1 sm:col-span-2">
              <Label htmlFor="addProductName">Naziv artikla *</Label>
              <Input
                id="addProductName"
                value={addForm.productName}
                onChange={(e) => setAddForm((f) => ({ ...f, productName: e.target.value }))}
                placeholder="npr. Omega-3 kapsule"
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); document.getElementById("addMinQty")?.focus(); }}}
              />
            </div>
            <div className="space-y-1">
              <Label htmlFor="addMinQty">Minimalna količina *</Label>
              <Input
                id="addMinQty"
                type="number"
                min={1}
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`rounded-xl border bg-card overflow-hidden ${done ? "border-emerald-200" : ""}`}
              >
                {/* Header artikla */}
                <div className={`px-5 py-4 ${done ? "bg-emerald-50/50" : ""}`}>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Input
                      ...
                      value={sf.qty}
                      onChange={(e) =>
                        setSaleForm((prev) => ({ ...prev, [item.id]: { ...sf, qty: e.target.value } }))
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                    ...
                    <Input
                      ...
                      value={sf.note}
                      onChange={(e) =>
                        setSaleForm((prev) => ({ ...prev, [item.id]: { ...sf, note: e.target.value } }))
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          handleAddSale(item.id);
                        }
                      }}
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                <p className="text-xs text-muted-foreground truncate">
                                  {entry.enteredBy.name !== "" ? entry.enteredBy.name : "Nepoznat"} · {formatDate(entry.enteredAt)}
                                </p>
                                {entry.note && (
